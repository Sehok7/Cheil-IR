...
당사(제일기획)와 함께 국내 경쟁사(이노션), KOSPI/벤치마크 지수, 글로벌 광고대행사 4사(Publicis, Dentsu, WPP, Omnicom), 주요 환율을 종합적으로 본 인사이트를 작성해주세요.
...
[국내 벤치마크 지수]
${indexInfo}
※ 일반서비스 지수는 한국거래소(KRX) 공식 Open API에서 직접 수집된 데이터.
...
6. 당사 vs 이노션 상대 비교는 IR 보고에서 중요 — 누적 수익률 차이를 명시적으로 다룰 것.

[출력 형식 - JSON으로만 응답]
{
  "market_summary": "오늘 광고업계(국내+글로벌) 시장 동향을 5~6문장으로 요약. 당사·이노션·지수·글로벌 4사 일일 등락 + 12/1 이래 누적 추세 + 환율 + 주요 이슈 종합.",
  "exec_briefing": [
    "당사(제일기획) 일일 등락 + 12/1대비 누적 수익률 핵심 포인트",
    "국내 경쟁사(이노션)와의 상대 강도 비교 (누적 수익률 갭 강조)",
    "글로벌 4사 중 주목할 종목 동향 (1~2개 픽업)",
    "환율/매크로 또는 종합 메시지"
  ],
  "stock_cause": "당사(제일기획) 주가 변동의 추정 원인을 뉴스/거래량/지수 흐름/환율 영향 기반으로 3~4문장 분석. ①, ②, ③ 형태로 원인 나열."
}
...
JSON만 출력하고 다른 텍스트는 포함하지 마세요.`;
  ...
  try {
    ...
    return {
      ...
      exec_briefing: [
        `당사 ${main.name}: ${fmtPct(main.change_pct)} (거래량 ${main.volume.toLocaleString('ko-KR')})`,
